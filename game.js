...
for(var i=0; i<16; i++)                             //blcok chessboard
{
    chessboard[i] = new Array(16);
    for(var j=0; j<16; j++)
    {
        chessboard[i][j] = 0;
    }
...
window.onload = function Initboard()
{
    ...
    for(var i=0; i<15; i++)
    {
        for(var j=0; j<15; j++)
        {
            context.strokeRect(27+j*37, 27+i*37, 37, 37);
        }
    }
}
...
{
    var x = ((e.offsetX-27)/37);                     //get x position
    var y = ((e.offsetY-27)/37);                     //get y position
...
function oneStep(drop_x, drop_y)
{
    ...
    context.arc(27+37*drop_x, 27+37*drop_y, 14, 0, 2*Math.PI); 
